/**
 * Actuator blueprint for a RGB digital pin.
...
export default {
  ...
  if ($id_currFrameIndex < $id_SIZE) {
    ...
    if (currentFrame == $id[$id_currFrameIndex].start) {
      digitalWrite($id_PIN1, $id[$id_currFrameIndex].r);
      digitalWrite($id_PIN2, $id[$id_currFrameIndex].g);
      digitalWrite($id_PIN3, $id[$id_currFrameIndex].b);
    }
  }
  `,
